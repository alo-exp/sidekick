...
- **`tests/run_all.bash`**, **`tests/run_release.bash`**, **`README.md`**, **`context.md`**, **`site/CICD.md`**, **`site/TESTING.md`**, **`site/pre-release-quality-gate.md`**: testing docs and automation now distinguish strict local validation, skip-safe local sweeps, and live release authorization.
...
- **`hooks/validate-release-gate.sh`**: release authorization now blocks release tag pushes, including `git push origin vX.Y.Z`, `refs/tags/*`, `--tags`, `--follow-tags`, `--mirror`, dynamic tag refspecs, and git aliases that expand to tag pushes, until all required gate evidence is present.
...
- **`hooks/validate-release-gate.sh`**: release authorization now also detects inherited release endpoint variables, GitHub Enterprise-style API URLs, unicode-escaped tag refs, curl config-file writes, field-file tag refs, and nested interpreter release-tag pushes.
- **`hooks/validate-release-gate.sh`**: release authorization now also covers same-command generated curl configs and shell scripts, process-substitution curl configs, command-scoped release URL env vars, generic dynamic release refs, GHES `/api/graphql`, chained git aliases, direct executable scripts without release-themed names, `git -C` target repos, and explicit `gh release create --target` refs.
- **`hooks/validate-release-gate.sh`**: release authorization now fails closed for ambiguous release target provenance, including different-repo `gh -R`/`gh api` writes, GraphQL tag mutations without repo identity, command-scoped `HOME`/`XDG_CONFIG_HOME`/`GIT_CONFIG_GLOBAL` alias sources, cwd-changing wrappers, and unverified tag-backed `gh release create` commands.
- **`hooks/validate-release-gate.sh`**: release authorization now binds `gh` repo/host provenance to GitHub.com `alo-exp/sidekick`, validates actual `git push` tag destinations including `pushurl` and persistent Git URL rewrites, rejects untrusted git/gh command-scoped context, requires trusted Sidekick checkout provenance for release ref resolution, requires explicit `target_commitish` for `gh api` release creates, and treats `--verify-tag=false` as unverified even when `--target` is present.
- **`hooks/validate-release-gate.sh`**, **`hooks/hooks.json`**, **`site/pre-release-quality-gate.md`**, **`site/internal/pre-release-quality-gate.md`**: release authorization now denies hidden release commands inside substitutions, broader same-command write redirections such as `>|` and `<>`, and command-substituted release targets; release runbooks now require a literal current SHA target, and packaged hooks prefer active host plugin roots before stale generic roots.
- **`hooks/validate-release-gate.sh`**, **`site/pre-release-quality-gate.md`**, **`site/CICD.md`**: release authorization now requires explicit `--repo alo-exp/sidekick` for `gh release create`, denies truthy `--verify-tag` even with a literal target, rejects destructive `gh api` release/tag-ref item endpoints with marker-complete target fields, and verifies an existing remote release tag already points at the current trusted HEAD before allowing target-based release creation.
- **`hooks/validate-release-gate.sh`**: release authorization now rejects `GH_REPO` as a substitute for the documented `--repo alo-exp/sidekick` release command, detects file-backed `force:true` tag-ref API payloads, and sanitizes release-sensitive env in hook regression tests.
- **`hooks/validate-release-gate.sh`**, **`tests/test_validate_release_gate_hook.bash`**, **`site/pre-release-quality-gate.md`**: release authorization now treats `--` as the end of `gh` provenance options, rejects duplicate REST target/force fields, checks existing remote tags for `gh api .../git/refs` and `git push HEAD:refs/tags/...`, and documents that Stage 1 uses the actual `/superpowers:requesting-code-review` reviewer workflow.
- **`hooks/validate-release-gate.sh`**, **`tests/test_validate_release_gate_hook.bash`**: release authorization now rejects git-config-driven tag publication, bare local non-semver tag pushes, same-command non-semver tag creation plus push, and same-command file writes before release creation, including multi-redirect and nested shell payload forms.
...
- **Release-gate bypasses**: blocked additional shell expansion, shell/GitHub CLI alias, persistent `gh` alias without executing `gh` during classification, `case`, interpreter inline/stdin/heredoc/argv-vector payload, array/positional, literal shell-variable `bash -c` payload, REST `gh api`, direct GitHub REST release/tag writes through `curl`/`wget`/interpreter payloads, and GraphQL release/tag mutation paths before quality-gate markers are complete.
- **Sidekick switching**: Forge and Kay activation are now mutually exclusive per host session via a shared `active-sidekick` marker, so switching sidekicks does not leave both PreToolUse hooks enforcing at once.
- **Website asset stability**: the homepage now pins Lucide to the same SRI-protected version as the Help Center and guards icon initialization when the CDN is unavailable.
