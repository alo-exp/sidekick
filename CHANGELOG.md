...
- **`hooks/validate-release-gate.sh`**, **`tests/test_validate_release_gate_hook.bash`**: release authorization now rejects git-config-driven tag publication, bare local non-semver tag pushes, same-command non-semver tag creation plus push, and multi-redirect same-command file writes before release creation.
