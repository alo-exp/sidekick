...
- **`hooks/validate-release-gate.sh`**: release authorization now binds `gh` repo/host provenance to GitHub.com `alo-exp/sidekick`, validates `git push` tag remotes, rejects command-scoped `GH_CONFIG_DIR` and same-command `gh` aliases as release target evidence, requires explicit `target_commitish` for `gh api` release creates, and treats `--verify-tag=false` as unverified.
