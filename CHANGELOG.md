...
- **`hooks/validate-release-gate.sh`**: release authorization now binds `gh` repo/host provenance to GitHub.com `alo-exp/sidekick`, validates actual `git push` tag destinations including `pushurl`, rejects untrusted git/gh command-scoped context, requires trusted Sidekick checkout provenance for release ref resolution, requires explicit `target_commitish` for `gh api` release creates, and treats `--verify-tag=false` as unverified even when `--target` is present.
