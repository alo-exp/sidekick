...
[`site/pre-release-quality-gate.md`](../pre-release-quality-gate.md#stage-1--code-review-triad).
...
- **Token budget**: The 2,000-token task prompt cap appears consistently in SKILL.md, `site/help/reference/index.html`, and `site/help/concepts/index.html`.
...
Audit all `site/help/*/index.html` pages:

- **Step counts**: The numbered steps in `site/help/workflows/index.html` match the actual steps in SKILL.md's delegation protocol. No steps added or removed without being reflected in the workflow page.
...
- **Config paths**: Every file path mentioned in help pages exists or is correctly described: `~/forge/.credentials.json`, `~/forge/.forge.toml`, the active host session marker under the resolved host session id (`SIDEKICK_SESSION_ID`, then host-specific session id, then `SESSION_ID`), `.forge/skills/*/SKILL.md`, `site/sessions/`
...
- **Quick-links grid**: All 10 quick links in `site/help/index.html` resolve to existing anchors on existing pages
...
- **Three-tier write correctness**: Verify writes go to exactly three targets after each task: `./AGENTS.md`, `~/forge/AGENTS.md`, `site/sessions/YYYY-MM-DD-session.md`. No other paths.
- **Deduplication**: Verify the two-phase dedup (exact substring + semantic similarity) is described consistently in SKILL.md and `site/help/concepts/index.html`
...
### Step 4 — All site/help/*.html Pages
