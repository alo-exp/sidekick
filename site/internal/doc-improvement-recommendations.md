...
`site/knowledge/` and `site/lessons/` are good memory layers, but they are not a substitute for structured decisions.
...
That can live in `site/ADR/` or a similar decision-log area.
