...
**What to expect:** Structured items appended to `site/tech-debt.md` in the format:
`| Item | Severity | Effort | Phase introduced |`. The file is created if it does not exist.

**If it fails:** Review the tech debt items manually. Ensure `site/tech-debt.md` is writable.
...
- `site/PRD-Overview.md` -- sync high-level areas from `.planning/REQUIREMENTS.md`
- `site/ARCHITECTURE.md` -- high-level architecture and principles only;
  detailed phase designs live in `site/specs/`
- `site/TESTING.md`
- `site/CICD.md`

**Additional required updates at this step:**
- Update `site/knowledge/YYYY-MM.md`: append dated entries under Architecture patterns,
...
- Update `site/lessons/YYYY-MM.md` when a portable (non-project-specific) lesson emerged.
- Update `site/CHANGELOG.md`: prepend a new entry (newest first):
  ```
...
- Complete the session log: read path from `~/.claude/.silver-bullet/session-log-path`,
  ...
  create `site/sessions/<today>-manual.md` from the session log template.
- Documentation agents writing to `site/` run in the **main worktree only**
  (no `isolation: "worktree"`). Only implementation-touching agents use worktree isolation.

**If it fails:** Check file permissions on the `site/` directory. Re-run targeting specific
...
3. The DevOps workflow (`site/workflows/devops-cycle.md`) takes over with infrastructure-specific steps including blast radius analysis, environment promotion, and incident response
...
| Design specs | Superpowers | Save to `site/specs/YYYY-MM-DD-<topic>-design.md`. Superpowers' default path (`site/superpowers/specs/`) is NOT used -- always override it. |
...
- Spec save path: `site/specs/` (not `site/superpowers/specs/`)
