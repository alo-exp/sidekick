...
**Current milestone:** v0.6.2. All 77 requirements across v1 + v1.2 remain validated.

---

## Product Vision

Sidekick is a Claude Code and Codex plugin that turns the host AI into a **planner, reviewer, and mentor** and delegates implementation work — file writes, edits, shell mutations, git operations — to execution agents. The shipped sidekicks are **Forge** (ForgeCode), **Kay** (OSS Codex-lineage, `kay` binary), and **Codex** (the local OpenAI Codex CLI). Target user: a developer using Claude Code or Codex who wants capable, lower-cost execution agents handling implementation while the host handles thinking and the conversation.

---

## Core Value

**When Forge or Kay delegation is active, the host does not write code directly.** Delegation is harness-enforced (PreToolUse hook returns `permissionDecision: "deny"` on direct `Write`/`Edit`/`NotebookEdit`), agent subprocess output is surfaced as a bounded, redacted transcript view with stable `[FORGE]` / `[KAY]` markers, and every task is durably indexed in `.forge/conversations.idx` or `.kay/conversations.idx`. The ONE thing that must work above all else: **delegation cannot be bypassed, and every task is traceable.**
...
| Delegation protocol | `DLGT-01..05` | 5-field task prompt composition, submission, monitoring; the host AI never writes code while active (except L3 fallback) |
| Fallback ladder | `FALL-01..05` | L1 Guide (reframe + retry), L2 Handhold (atomic decomposition, max 3), L3 Take over (`sidekick forge-level3 start|stop`, project-scoped direct tool use + DEBRIEF) |
...
### v1.2 — Forge Delegation + Bounded Visibility (v1.2.0)
...
| Bounded visibility | `VIS-01..04` | safe-runner transcript view with redaction, output caps, and foreground fallback for Bedrock/Vertex/Foundry |
...
| Plugin manifest | `MAN-01..04` | Current release metadata remains `v0.6.2`, hooks registered, commands + output styles registered, `_integrity` refreshed |
| Delegation coverage | `TEST-V12-01..05` | Current enforcer hook, progress surface, UUID format, history pruning, and full delegation-flow assertions remain validated in the v0.6.2 test suite |
...
| Removing `skills/forge.md` | The canonical Forge workflow now lives in `skills/forge/SKILL.md`; the flat file remains as a hidden compatibility wrapper. |
...
| Automatic session-end AGENTS.md extraction | No Forge built-in for this; the host AI drives extraction explicitly at task completion. |
...
- **Source of truth for requirements:** `.planning/milestones/v1.4.0-REQUIREMENTS.md` (77 requirements, all validated as of v0.6.2).
