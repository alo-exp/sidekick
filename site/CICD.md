...
3. **Pre-tag live gate** — maintainer runs the Codex live release pyramid twice against that same commit; Forge live stages may be added when available:
   ...
   This chains tier 1 (`run_unit.bash`) → tier 2 (`smoke/run_smoke.bash`) → tier 3 (`run_live_e2e.bash`) → tier 4 (`run_live_codex_marketplace_install.bash`) → tier 5 (`smoke/run_codex_smoke.bash`) → tier 6 (`run_live_codex_e2e.bash`) with fail-fast aborts. Each live Codex run writes an isolated `quality-gate-live-pyramid-candidate` marker, and `tests/run_in_kay.bash` promotes successful canonical runs to proof-bound final `quality-gate-live-pyramid` markers. The release hook requires two final markers before it allows release tag publication or `gh release create`. Forge live stages can be added when available, but they are not required when Forge testing is intentionally skipped.
