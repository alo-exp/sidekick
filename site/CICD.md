...
Always-green for anything merged to `main`. Live Kay/Codex tiers are NOT run in CI; they are gated on `SIDEKICK_LIVE_CODEX=1`.
...
3. **Pre-tag live gate** — maintainer runs the Kay-hosted Codex live release pyramid twice against that same commit:
   ```bash
   bash tests/run_in_kay.bash SIDEKICK_LIVE_CODEX=1 bash tests/run_release.bash
   bash tests/run_in_kay.bash SIDEKICK_LIVE_CODEX=1 bash tests/run_release.bash
   ```
   This chains tier 1 (`run_unit.bash`) to the live Codex marketplace install, Kay smoke, and Kay live E2E checks with fail-fast aborts. Each live Codex run writes an isolated `quality-gate-live-pyramid-candidate` marker, and `tests/run_in_kay.bash` promotes successful canonical runs to proof-bound final `quality-gate-live-pyramid` markers. Release operators must verify two final markers before publishing a tag or GitHub release.
   Local/pre-release test evidence is produced inside Kay through `tests/run_in_kay.bash`. Kay/Codex stages force OpenCode Go with `deepseek-v4-flash` and low reasoning so release evidence is generated by the verifier/reporting model profile.
...
| Missing | Reason |
|---|---|
| Live Kay/Codex model calls in CI | Live sidekick calls require local credentials and can spend tokens; gating on `SIDEKICK_LIVE_CODEX` keeps CI runtime fast, deterministic, and zero-cost. |
