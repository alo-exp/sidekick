...
| **Project docs** | `site/` | Durable across milestones | Architecture, testing strategy, changelog, knowledge, lessons, help, internal audit material, workflow docs |
...
| Orchestration | `.claude/`, `hooks/`, `skills/`, `output-styles/`, `sidekicks/` | Sidekick behavior, enforcement, and user-facing entrypoints |
| Documentation | `site/` | Durable product, test, process, help, and internal audit docs |
...
| Doc | Purpose |
|-----|---------|
| `site/START-HERE.md` | Question-first navigation for install, delegate, debug, release, extend, and migrate |
| `site/AUDIENCE.md` | Reader matrix — who each doc is for and where they should begin |
| `site/GLOSSARY.md` | Canonical vocabulary for Sidekick, Kay, Codex, skills, selectors, bridges, and wrappers |
| `site/COMPATIBILITY.md` | Runtime matrix for Claude, Codex, and Kay |
| `site/ADR/README.md` | Entry point for durable decision records |
...
- `site/knowledge/` and `site/lessons/` hold append-only project memory
- `site/specs/` and `site/design/` preserve archived spec/design material when relevant
...
|-----------|---------|-------------|
| `site/knowledge/` | Project-scoped intelligence — architecture patterns, gotchas, key decisions, recurring issues | **Project-specific** — references this codebase directly |
| `site/lessons/` | Portable lessons learned — things useful beyond this project | **Portable** — no project-specific file paths or feature names |
...
| `workflows/` | `/silver` composer | Per-instance workflow trackers under `.planning/workflows/<id>.md`; legacy `WORKFLOW.md` is historical evidence only |
...
|----------|-----|-------------|
| `site/*.md` | 500 lines | Artifact reviewer flags violations |
| `site/knowledge/*.md`, `site/lessons/*.md` | 300 lines | Split into `YYYY-MM-a.md` / `YYYY-MM-b.md` if exceeded |
...
1. `site/` files summarize — `.planning/` artifacts are the source of truth during development
...
6. Per-instance workflow files under `.planning/workflows/` are the composition execution log — `silver-bullet.md` §2h describes the architecture, and each active workflow tracks its own instance file. Legacy `.planning/WORKFLOW.md` is historical evidence only.
