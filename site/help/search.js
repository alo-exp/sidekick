...
var IDX = [
  ...
  { page:'Audience', url:'../AUDIENCE.md', anchor:'audience',
    ...
    text:'Reader matrix for new users, maintainers, release operators, plugin authors, Claude Code users, Codex users, Kay users, and Kay operators.' },
  ...
  { page:'Compatibility', url:'../COMPATIBILITY.md', anchor:'compatibility',
    title:'Compatibility — Claude Code, Codex, Forge, and Kay',
    text:'Compatibility matrix covering host surfaces, execution agents, skill source of truth, command discoverability, and provider / execution identity.' },
  ...
  { page:'Help Center', url:'../', anchor:'support',
    ...
    text:'Claude Code and Codex can both route work to either agent: use /forge or /forge:delegate for Forge mode, and kay-delegate, sidekick:kay-delegate, or /kay:delegate for Kay mode. The legacy code alias remains compatibility-only. OpenCode Go remains Kay compatibility path with MiMo-V2.5-Pro for non-trivial work, MiMo-V2.5 for vision / visual reasoning, MiniMax M2.7 for trivial work, and DeepSeek V4 Flash for test running, issue reporting, and completion verification.' },

  // ── GETTING STARTED ───────────────────────────────────────────
  { page:'Getting Started', url:'getting-started/', anchor:'what-is-sidekick',
    title:'What is Sidekick?',
    text:'Sidekick is a Claude Code and Codex plugin that gives the active host two implementation agents. Claude Code and Codex can both route work to Forge or Kay; Kay uses the OpenCode Go path with MiMo-V2.5-Pro for non-trivial work, MiMo-V2.5 for vision / visual reasoning, MiniMax M2.7 for trivial work, and DeepSeek V4 Flash for test running, issue reporting, and completion verification.' },
  ...
  { page:'Getting Started', url:'getting-started/', anchor:'kay-path',
    ...
    text:'Start Kay mode through kay-delegate or sidekick:kay-delegate. Active Kay mode verifies Kay, creates the session marker, then routes child execution through kay exec --full-auto. Older binary names are compatibility-only. Kay preserves .kay/conversations.idx lookup metadata.' },
  ...
  { page:'Core Concepts', url:'concepts/', anchor:'kay-delegation',
    ...
    text:"Kay is Sidekick's execution agent for the OSS Codex lineage. Activation starts with kay-delegate or sidekick:kay-delegate; active Kay mode routes work through kay exec --full-auto. Sidekick adds package wiring, a session-scoped .kay marker, .kay/conversations.idx, and progress summaries. Kay task routing uses MiMo-V2.5-Pro for planning, implementation, reviewing, and other non-trivial work, MiMo-V2.5 for vision / visual reasoning, MiniMax M2.7 for trivial work, and DeepSeek V4 Flash for test running, issue reporting, and completion verification." },
  ...
  { page:'Core Concepts', url:'concepts/', anchor:'fallback-ladder',
    ...
    text:'Three-level recovery. Level 1 Guide reframe prompt diagnose failure tighter DESIRED STATE single retry. Level 2 Handhold decompose into atomic subtasks under 200 tokens sequential submission max 3 attempts total. Level 3 Take over uses sidekick forge-level3 start and stop to temporarily lift delegation inside the project boundary and produces DEBRIEF with AGENTS_UPDATE proposed for confirmation.' },
  ...
  { page:'Core Concepts', url:'concepts/', anchor:'provider-config',
    ...
    text:'Forge provider config lives in ~/forge/.forge.toml and credentials in ~/forge/.credentials.json. Kay uses OpenCode Go provider routing in Kay config, with MiMo-V2.5-Pro for non-trivial work, MiMo-V2.5 for vision / visual reasoning, MiniMax M2.7 for trivial work, and DeepSeek V4 Flash for test running, issue reporting, and completion verification. Use kay login --provider opencode-go --with-api-key for Kay credentials.' },
  ...
  { page:'Delegation Workflow', url:'workflows/', anchor:'kay-workflow',
    ...
    text:'Kay workflow verifies the Kay runtime and OpenCode Go login, activates Kay mode through kay-delegate or sidekick:kay-delegate, clears any Forge marker writes active-sidekick=kay, then uses the native kay exec --full-auto runtime path while reviewing Kay progress summary and .kay/conversations.idx lookup metadata.' },
  ...
  { page:'Delegation Workflow', url:'workflows/', anchor:'fallback',
    ...
    text:'L1 Guide diagnose rewrite prompt resubmit once. L2 Handhold decompose atomic subtasks submit sequentially up to 3 attempts total. L3 Take over host enables sidekick forge-level3 start, implements directly inside project boundary, stops Level 3, and produces DEBRIEF with TASK FORGE_FAILURE LEARNED AGENTS_UPDATE fields. AGENTS_UPDATE is proposed for confirmation.' },
  { page:'Delegation Workflow', url:'workflows/', anchor:'agents-update',
    title:'Post-task AGENTS.md update',
    text:'After every Forge task completion the host extracts learnings and writes only new deduplicated AGENTS.md guidance. Successful tasks can contribute preferences and project patterns. L3 takeovers produce DEBRIEF AGENTS_UPDATE text for confirmation. Session log written to site/sessions/.' },

  // ── COMMAND REFERENCE ─────────────────────────────────────────
  { page:'Reference', url:'reference/', anchor:'forge-commands',
    title:'Delegation skills — forge and Kay lifecycle',
    text:'/forge activates Forge delegation mode for the current session and arms hooks after health check. /forge:delegate is an alias for /forge. /forge-stop removes the Forge marker. kay-delegate or sidekick:kay-delegate uses the Kay workflow for kay runtime tasks. /kay:delegate is an alias for kay-delegate. /kay-stop removes the Kay marker.' },
  { page:'Reference', url:'reference/', anchor:'kay-commands',
    title:'Kay runtime commands',
    text:'kay exec --full-auto is the native runtime command used after Kay mode is active. kay login --provider opencode-go --with-api-key configures OpenCode Go credentials. Older binary names are compatibility-only.' },
  { page:'Reference', url:'reference/', anchor:'forge-toml',
    title:'.forge.toml config — model credentials compaction providers',
    text:'.forge.toml created on first /forge in project root if absent. Contains compaction settings only: max_tokens 16384, compact token_threshold 80000, eviction_window 0.20, retention_window 6. API credentials stored globally in ~/forge/.credentials.json as array entries with id and auth_details, and provider/model config lives in ~/forge/.forge.toml. Forge highlights minimax with MiniMax-M2.7.' },
  { page:'Getting Started', url:'getting-started/', anchor:'prerequisites',
  ...
  { page:'Troubleshooting', url:'troubleshooting/', anchor:'health-check',
    ...
    text:'Forge provider or credentials not valid: check forge binary, forge info, ~/forge/.credentials.json array of id and auth_details entries, and ~/forge/.forge.toml provider_id and model_id. Model not available: verify the minimax MiniMax-M2.7 provider pair. Bootstrap skills not loaded: reinstall /plugin install alo-labs/sidekick.' },
  { page:'Troubleshooting', url:'troubleshooting/', anchor:'kay-runtime',
    title:'Kay runtime issues — kay exec unavailable or Kay provider login missing',
    text:"If kay exec is unavailable, check kay --version and kay exec --help. Reinstall Sidekick or the Kay runtime if the primary binary is unavailable. If the Kay OpenCode Go provider route is missing credentials, run kay login --provider opencode-go --with-api-key." },
  ...
  { page:'Troubleshooting', url:'troubleshooting/', anchor:'recovery',
    ...
    text:'Stuck session deactivate and reactivate: /forge-stop then /forge. After L3 takeover the Forge marker remains active for next task and the Level 3 marker should be stopped. If the host implements directly instead of delegating say re-enter Forge delegation mode.' },
  { page:'Troubleshooting', url:'troubleshooting/', anchor:'reinstall',
    title:'Reinstall and full reset — clean install, credentials reset',
    text:'Clean reinstall: /plugin install alo-labs/sidekick preserves AGENTS.md and site/sessions/. Reset credentials: delete ~/forge/.credentials.json then /forge to guide through setup again. AGENTS.md and session logs always preserved.' },
];
