...
| Commands | `commands/forge-stop.md`, `commands/forge-history.md`, `commands/codex-stop.md`, `commands/codex-history.md` | Canonical activation/deactivation and history workflow docs. Claude exposes them as commands; Codex exposes the Code workflows again as skill bridges under `skills/codex-stop/SKILL.md` and `skills/codex-history/SKILL.md` because its manifest does not load custom command entries. This preserves the official Codex pattern of skills-first workflows while keeping the shared docs single-sourced. |
...
| Codex plugin manifest | `.codex-plugin/plugin.json` | v1.5.1. Registers hooks and skills for Codex. Command workflows are surfaced through Codex skills rather than a manifest `commands` field. |
...
- **New sidekick** (additional coding agent): add `skills/<name>/SKILL.md`, optional `commands/<name>-*.md`, `output-styles/<name>.md`, and a row in `sidekicks/registry.json`. If the target runtime does not support custom command manifests, expose the same workflows as skill bridges that point back to the canonical command docs. The shared hook library and manifest wiring should not need to change.
