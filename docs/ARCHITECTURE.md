...
| Commands | `commands/forge-stop.md`, `commands/forge-history.md`, `commands/codex-stop.md`, `commands/codex-history.md` | Sidekick-specific activation/deactivation and history surfaces. Each command manages only its own marker/index. |
...
- **New sidekick** (additional coding agent): add `skills/<name>/SKILL.md`, optional `commands/<name>-*.md`, `output-styles/<name>.md`, and a row in `sidekicks/registry.json`. The shared hook library and manifest wiring should not need to change.
