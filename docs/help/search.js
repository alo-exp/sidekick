...
var IDX = [
  ...
  { page:'Help Center', url:'../', anchor:'support',
    ...
    text:'Claude Code users delegate to Forge. Code and Codex workflows route to Kay through code exec, with codex and coder compatibility aliases.' },

  // ── GETTING STARTED ───────────────────────────────────────────
  { page:'Getting Started', url:'getting-started/', anchor:'what-is-sidekick',
    title:'What is Sidekick?',
    text:'Sidekick is a Claude Code and Codex plugin that gives the active host a specialized execution runtime. Forge is the Claude Code sidekick. Kay is the Code/Codex path built on the Every Code runtime with MiniMax M2.7 defaults.' },
  { page:'Getting Started', url:'getting-started/', anchor:'prerequisites',
    title:'Prerequisites — host and provider key',
    text:'Required: Claude Code for Forge, Code or Codex host for Kay, provider key for OpenRouter or MiniMax, Node.js 18 or later for Claude Code. Sidekick auto-installs missing Forge and Kay assets on SessionStart.' },
  { page:'Getting Started', url:'getting-started/', anchor:'install',
    title:'Installing Sidekick',
    text:'Install Sidekick inside Claude Code with /plugin install alo-labs/sidekick. Makes /forge skill available in all sessions. Works in any git repository. Codex users get the matching Codex plugin surface under the same Sidekick package identity.' },
  { page:'Getting Started', url:'getting-started/', anchor:'health-check',
    title:'Forge health check — verify connectivity',
    text:'Run /forge to activate and run health check. On first activation checks ~/forge/.credentials.json for API key and ~/forge/.forge.toml for provider config. Kay users verify code --version, code exec --help, and MiniMax login.' },
  { page:'Getting Started', url:'getting-started/', anchor:'kay-path',
    title:'Your first Kay task',
    text:'Kay uses Code native execution. Use code exec --full-auto first, then codex exec --full-auto or coder exec --full-auto as compatibility fallbacks. Kay preserves .kay/conversations.idx lookup metadata.' },
  ...
  { page:'Core Concepts', url:'concepts/', anchor:'kay-delegation',
    title:'Kay delegation model — Code native execution',
    text:'Kay is the Sidekick identity for the Every Code runtime. Primary path is code exec --full-auto with codex and coder fallbacks. Sidekick adds package wiring SessionStart sync .kay marker .kay/conversations.idx and progress summaries.' },
  ...
  { page:'Core Concepts', url:'concepts/', anchor:'provider-config',
    ...
    text:'Forge provider config lives in ~/forge/.forge.toml and credentials in ~/forge/.credentials.json. Kay defaults to MiniMax M2.7 in ~/.code/config.toml with legacy ~/.codex/config.toml compatibility. Use code login --provider minimax --with-api-key for Kay credentials.' },
  ...
  { page:'Delegation Workflow', url:'workflows/', anchor:'kay-workflow',
    ...
    text:'Kay workflow verifies code runtime and MiniMax login, runs code exec --full-auto task or codex/coder fallback, then reviews Kay progress summary and .kay/conversations.idx lookup metadata.' },
  ...
  { page:'Reference', url:'reference/', anchor:'kay-commands',
    title:'Kay runtime commands — code, codex, coder',
    text:'code exec --full-auto is the primary Kay execution command. codex exec --full-auto and coder exec --full-auto are compatibility fallbacks. code login --provider minimax --with-api-key configures MiniMax credentials.' },
  ...
  { page:'Reference', url:'reference/', anchor:'file-structure',
    ...
    text:'skills/forge/SKILL.md and skills/codex-delegate/SKILL.md instruction bodies. .forge.toml compaction config; credentials live in ~/forge/.credentials.json. .forge and .kay indexes preserve task lookup metadata. ~/.code/config.toml stores Kay config with legacy ~/.codex compatibility.' },
  ...
  { page:'Troubleshooting', url:'troubleshooting/', anchor:'kay-runtime',
    title:'Kay runtime issues — code exec unavailable or MiniMax login missing',
    text:'If code exec is unavailable, check code --version, codex --version, coder --version and matching exec --help output. Restart host app for runtime sync repair. If MiniMax login is missing, run code login --provider minimax --with-api-key.' },
