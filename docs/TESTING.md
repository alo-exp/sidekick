...
Three tiers, fail-fast, each with a distinct purpose. The lower a failure appears in the pyramid, the cheaper it is to find.
...
Stages 2 and 3 are gated behind `SIDEKICK_LIVE_FORGE=1` so they never run in CI. Without the env var, both stages exit 0 cleanly and the release gate still runs stage 1.
