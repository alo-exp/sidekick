...
expect_contains "README.md" '| **Codex** | `codex-delegate` |' "README uses codex-delegate activation surface"
...
expect_contains "context.md" 'Codex activates through `codex-delegate` / `sidekick:codex-delegate`' "context names Codex activation surface before runtime command"
...
expect_contains "context.md" 'Codex sidekick markers live under `.codex/sessions/...`.' "context documents Codex marker root"
expect_contains "context.md" '.codex/conversations.idx' "context documents Codex audit index"
...
expect_not_contains "site/internal/pre-release-quality-gate.md" "git push origin v<version>" "internal release gate avoids pre-pushing release tags"
...
expect_contains "site/pre-release-quality-gate.md" '--target <current-sha>' "release gate documents literal release target"
expect_contains "site/internal/pre-release-quality-gate.md" '--target <current-sha>' "internal release gate documents literal release target"
expect_contains "site/CICD.md" '--target <current-sha>' "CI/CD docs document literal release target"
expect_not_contains "site/pre-release-quality-gate.md" '--target "$(git rev-parse HEAD)"' "release gate avoids shell-substituted release target"
expect_not_contains "site/internal/pre-release-quality-gate.md" '--verify-tag' "internal release gate avoids denied verify-tag release flow"
expect_not_contains "site/CICD.md" '--target "$(git rev-parse HEAD)"' "CI/CD docs avoid shell-substituted release target"
