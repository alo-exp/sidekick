...
router.get('/', (req, res) => {
  ...
  const orderBy = SORT_MAP[sortKey] || SORT_MAP.updated_desc;
  sql += ` ORDER BY pinned DESC, ${orderBy}`;
...
router.get('/export', (req, res) => {
  ...
  res.json(notes.map(noteRowToApi));
});
...
router.post('/import', (req, res) => {
  ...
  if (!Array.isArray(notes)) {
    return res.status(400).json({ error: 'Request body must be a JSON array or { notes: [...] }' });
  }
  ...
  const insertMany = db.transaction((items) => {
    for (const note of items) {
      ...
      const pinned = parseBool(note.pinned) ? 1 : 0;
      const archived = parseBool(note.archived) ? 1 : 0;
      const result = insert.run(title, body, tags, pinned, archived);
      const row = db.prepare('SELECT * FROM notes WHERE id = ?').get(result.lastInsertRowid);
      imported.push(noteRowToApi(row));
    }
  });

  insertMany(notes);
  res.status(201).json({ imported: imported.length, notes: imported });
});
...
router.post('/bulk-archive', (req, res) => {
  ...
  const validIds = ids.filter((id) => Number.isInteger(id) && id >= 1);
  if (validIds.length !== ids.length) {
    return res.status(400).json({ error: 'ids must be an array of positive integers' });
  }

  const db = getDb();
  const stmt = db.prepare('UPDATE notes SET archived = 1, updated_at = datetime(\'now\') WHERE id = ?');
  const archiveMany = db.transaction((noteIds) => {
    let count = 0;
  ...
  const archived = archiveMany(validIds);
  res.json({ archived });
});
