...
expect_contains "docs/og-image.html" "Codex lineage</span> + ForgeCode" "og image qualifies the benchmark headline through upstream lineage"
expect_contains "docs/og-image.html" "as</span> Sidekicks" "og image uses capital-S Sidekicks headline"
expect_not_contains "docs/og-image.html" "#1 and #2</span> coding agents" "og image avoids old rank-framing headline"
expect_not_contains "docs/og-image.html" "coding agents as your sidekicks" "og image avoids lowercase sidekicks headline"
expect_contains "docs/og-image.html" "#6 Codex CLI lineage" "og image highlights Kay upstream #6 result"
expect_contains "docs/og-image.html" "#7 ForgeCode" "og image highlights ForgeCode #7 result"
expect_contains "docs/og-image.html" "82.0%" "og image includes Codex score"
expect_contains "docs/og-image.html" "81.8%" "og image includes ForgeCode score"
expect_contains "docs/og-image.html" "Claude Code and Codex can delegate to Kay (OSS Codex lineage) or Forge" "og image states shared host-to-agent support concisely"
expect_contains "docs/og-image.html" "Codex CLI #6, ForgeCode #7" "og image qualifies Terminal-Bench rank ownership"
...
kay_line="$(grep -n '<div class="rank">#6</div>' "${ROOT}/docs/og-image.html" | head -n1 | cut -d: -f1 || true)"
forge_line="$(grep -n '<div class="rank">#7</div>' "${ROOT}/docs/og-image.html" | head -n1 | cut -d: -f1 || true)"
